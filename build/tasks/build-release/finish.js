module.exports = function(grunt, config, parameters, done) {
	...
	try {
		...
		var data = fs.readFileSync('./release/concrete5-legacy-patch-5.6.3.5.1/web/concrete/config/version.php', 'utf8'),
		...
		fs.renameSync('./release/concrete5-legacy-patch-5.6.3.5.1/web', './release/' + directory);
		shell.pushd('release');
		...
		shell.exec('zip -r ' + directory + '.zip ' + directory, {silent: true, async: true}, function(code, output) {
			...
			shell.rm('-rf', './release/concrete5-legacy-patch-5.6.3.5.1');
			done();
		});
	}
	catch(e) {
